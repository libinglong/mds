...
在[案例1](../史诗级java低时延调优案例1/案例1.md)中，作者介绍了在分布式高并发的环境下解决复杂问题的思路。但真实情况是，为了便于理解，作者仍对问题做了一些简化。通过阅读本文，读者将了解到，不同问题可能会导致相同的结果，而这些结果叠加在一起，会使发现问题的过程更加扑朔迷离。

### 第一章 问题再现

在[案例1](../史诗级java低时延调优案例1/案例1.md)中我们解决了由于c3p0配置错误导致老年代内存增长过快，cms垃圾回收耗时较长的问题。此后继续监测服务运行。又发现了同样的情况。
